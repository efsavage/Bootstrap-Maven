...
%><<c:choose><c:when test="${!empty href}">a href="${href}"</c:when><c:otherwise>button</c:otherwise></c:choose> class="btn <c:if test="${block}">btn-block</c:if> <c:choose><
